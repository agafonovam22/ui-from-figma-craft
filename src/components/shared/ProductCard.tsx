...
const ProductCard: React.FC<ProductCardProps> = ({ 
  ...
  return (
    ...
    >
      <div 
        className="relative group rounded-lg overflow-hidden hover:shadow-lg transition-all duration-300 h-auto"
        style={{ 
      ...
      {/* Слайдер изображений */}
      <div 
        className="relative h-40 sm:h-48 md:h-52 lg:h-60 overflow-hidden transition-colors duration-300"
      ...
      <div className="mx-4 h-px bg-gray-200 mt-7"></div>
        ...
        {/* Информация о товаре */}
        <div className="p-3 md:p-4 pb-2.5 mt-4 flex flex-col justify-between transition-colors duration-300 min-h-[120px] md:min-h-[140px]" style={{ backgroundColor: 'transparent' }}>
          <div>
            {/* Статус наличия */}
            {(() => {
              if (!product.in_stock) {
            ...
            {/* Название товара */}
            <h3 className="text-gray-900 group-hover:text-white text-xs sm:text-sm md:text-base mb-2 md:mb-3 line-clamp-2 leading-relaxed transition-colors duration-300">
              {(() => {
          ...
          <div className="flex items-center justify-between">
            <span className="text-sm md:text-base font-bold text-gray-900 group-hover:text-white transition-colors duration-300">
              {product.price.toLocaleString()} ₽
            </span>
            
            <Button 
              className="bg-[#F53B49] hover:bg-[#e63946] text-white px-2 py-1 sm:px-3 sm:py-2 md:px-4 md:py-2 text-xs sm:text-sm"
