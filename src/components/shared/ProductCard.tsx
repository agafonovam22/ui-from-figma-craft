...
const ProductCard: React.FC<ProductCardProps> = ({ 
  ...
  return (
    ...
    >
      ...
      {/* Слайдер изображений */}
      <div 
        className={`relative ${imageSize === 'small' ? 'h-64' : 'h-60'} overflow-hidden transition-colors duration-300`}
                ...
                <CarouselItem key={imageIndex} className="h-full">
                  <div className={`h-full flex items-center justify-center ${imageSize === 'small' ? 'p-8' : 'p-4'}`}>
                    <LazyImage 
                      ...
                      className={`${imageSize === 'small' ? 'max-w-[200px] max-h-44' : 'w-full h-full'} object-contain`}
                    />
                  </div>
                </CarouselItem>
              ))}
