  ...
  return (
    <section className="w-full py-6 bg-white">
      <div className="max-w-[1800px] mx-auto px-[30px]">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-6 gap-[10px] mb-6 hd:bg-red-100">
          {currentProducts.slice(0, 6).map((product) => (
            <div key={product.id} className="hd:w-[228px] hd:h-[281px] hd:border-4 hd:border-red-500">
              <ProductCard product={product} variant="grid" linkTo={product.linkTo} />
            </div>
          ))}
        </div>
        
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-6 gap-[10px] hd:bg-blue-100">
          {currentProducts.slice(6, 10).map((product) => (
            <div key={product.id} className="hd:w-[228px] hd:h-[281px] hd:border-4 hd:border-blue-500">
              <ProductCard product={product} variant="grid" linkTo={product.linkTo} />
            </div>
          ))}
