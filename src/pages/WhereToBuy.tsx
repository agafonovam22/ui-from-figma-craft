  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <main className="max-w-[1800px] mx-auto px-2 sm:px-4 lg:px-[60px] py-8">
        ...
        {/* Page Title */}
        <h1 className="font-heading text-[48px] text-layout-dark-grey mb-10 leading-none md:text-[36px] md:mb-6">
          Где купить
        </h1>

        {/* City Selector and Tabs */}
        <div className="flex flex-wrap items-center justify-between gap-4 h-[52px] mb-10 md:flex-col md:items-start md:h-auto md:mb-6 md:gap-4">
          <select 
            value={selectedCity} 
            onChange={(e) => setSelectedCity(e.target.value)}
            className="px-4 h-12 border border-[#CECFD7] rounded-md bg-white text-[12px] text-gray-700 md:w-full"
          >
          ...
          <div className="flex gap-2 md:flex-wrap md:w-full">
            <button
              onClick={() => setActiveTab('stores')}
              className={`px-6 h-12 rounded-md text-[12px] font-medium transition-colors md:flex-1 md:px-3 ${
                activeTab === 'stores'
        ...
        <div className="mb-8 md:mb-6">
          ...
          <p className="text-gray-700 text-lg leading-relaxed text-left w-full md:text-base">
            Приходите, тестируйте и убедитесь: только оригиналы, высочайшее качество!
          </p>
        </div>

        {/* Show on Map Toggle */}
        <div className="flex justify-end mb-6 md:justify-start">
        ...
        <div className="mb-12 md:mb-8">
          {showMap || sortBy === 'map' ? (
            <div>
              <h2 className="text-2xl font-bold text-gray-900 mb-6 md:text-xl md:mb-4">Магазины на карте</h2>
            ...
            <div className="bg-gray-50 rounded-lg flex h-[328px] overflow-hidden md:flex-col md:h-auto">
              ...
              <div className="w-2/3 h-full md:w-full md:h-48">
                <img 
        ...
        {activeTab === 'partners' && (
          <section className="mb-12 md:mb-8">
            <h2 className="text-2xl font-bold text-gray-900 mb-6 md:text-xl md:mb-4"><span style={{ color: '#778093' }}>Магазины</span> партнеров</h2>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-4">
              {partnerStores.map((store, index) => (
                <div key={index} className="bg-gray-50 rounded-lg p-6 md:p-4">
                  <h3 className="text-xl font-bold text-gray-900 mb-4 md:text-lg md:mb-3">{store.name}</h3>
                  
                     <div className="space-y-3 text-gray-700 md:space-y-2">
                      <div className="flex items-start gap-2">
                        <img src="/lovable-uploads/f0b02b09-ceb0-462c-a71b-75c67b2c6288.png" alt="Адрес" className="w-4 h-4 mt-1 flex-shrink-0" />
                       <div>
                         <p className="font-medium text-sm md:text-xs">Адрес</p>
                     ...
                     <div className="flex items-center gap-2">
                       ...
                       <div>
                         <p className="font-medium text-sm md:text-xs">Телефон</p>
                         <div className="text-sm md:text-xs">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        <p className="font-medium text-sm md:text-xs">Сайт</p>
                        <p className="text-sm md:text-xs">{store.hours}</p>
                      </div>
                    </div>
                  </div>
                </div>
        ...
        {activeTab === 'online' && (
          <section className="mb-12 md:mb-8">
            <h2 className="text-2xl font-bold text-gray-900 mb-6 md:text-xl md:mb-4">Интернет-магазины</h2>
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4 mb-8 md:grid-cols-2 md:gap-3 md:mb-6">
              {onlineStores.map((store, index) => (
                  <div key={index} className="bg-gray-50 rounded-lg p-4 text-center md:p-3">
                    <a 
                      href={`https://${store}`} 
                      target="_blank" 
                      rel="noopener noreferrer"
                      className="text-gray-700 hover:text-[#F53B49] transition-colors font-heading text-[16px] md:text-sm"
                    >
                      {store}
                    </a>
                  </div>
